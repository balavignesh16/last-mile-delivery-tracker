...
| Agent location entry can be one-tap GPS-filled via the browser's native Geolocation API (no map, no third-party service), with manual entry kept as a fallback; the fill never auto-submits | `frontend/src/pages/agent/OperationsPage.tsx` (`handleUseCurrentLocation`) | `OperationsPage.test.tsx` (button hidden when unsupported, fills without submitting, friendly error on permission denial); live-verified with a real Chromium geolocation override | `docs/user-agent-management.md` |
